...
public final class RoundUpHandler implements TransactionHandler {
    ...
    public boolean canHandle(TransactionEvent event) {
        ...
        return (event.getEventType() == null || "SPARE_CHANGE_AGGREGATE".equals(event.getEventType()))
                && "Round up".equals(art);
    }
    ...
    private String buildComment(TransactionEvent event) {
        ...
        String transaction = JsonDetailExtractor.getDetailValue(event,
                Arrays.asList("Übersicht", "data", "Transaktion", "detail", "text"));
        if (transaction != null)
