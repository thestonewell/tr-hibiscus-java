...
/**
 * Handles savings plan execution transactions with payment and frequency
 * details
 */
public final class SavingsPlanHandler implements TransactionHandler {
    ...
    public boolean canHandle(TransactionEvent event) {
        ...
        return (event.getEventType() == null || "TRADING_SAVINGSPLAN_EXECUTED".equals(event.getEventType()))
                && "Sparplan".equals(art);
    }

    /**
     * Extract savings plan details and build comment with payment, asset, and
     * frequency
     */
    @Override
    ...
    private String buildComment(TransactionEvent event) {
        ...
        String sparplanStatus = JsonDetailExtractor.getDetailValue(event,
                Arrays.asList("Übersicht", "data", "Sparplan", "detail", "text"));
        if (sparplanStatus != null)
            comment.append("Sparplan: ").append(sparplanStatus).append("\n");

        String zahlung = JsonDetailExtractor.getDetailValue(event,
                Arrays.asList("Übersicht", "data", "Zahlung", "detail", "text"));
        if (zahlung != null)
        ...
        String gesamtsumme = JsonDetailExtractor.getDetailValue(event,
                Arrays.asList("Übersicht", "data", "Summe", "detail", "text"));
        if (gesamtsumme != null)
