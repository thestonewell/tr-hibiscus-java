...
public final class CardPaymentHandler implements TransactionHandler {
    ...
    public boolean canHandle(TransactionEvent event) {
        ...
        return event.getEventType() == null && "Kartenzahlung".equals(art);
    }
    ...
    public TransactionData extractData(TransactionEvent event) {
        ...
        String empfaengerName = JsonDetailExtractor.getDetailValue(event, Arrays.asList("Übersicht", "data", "Händler", "detail", "text"));
    ...
    private String buildComment(TransactionEvent event) {
        String betrag = JsonDetailExtractor.getDetailValue(event, Arrays.asList("Übersicht", "data", "Betrag", "detail", "text"));
        if (betrag == null) return "";

        StringBuilder comment = new StringBuilder();
        comment.append("Betrag: ").append(betrag).append("\n");

        String wechselkurs = JsonDetailExtractor.getDetailValue(event, Arrays.asList("Übersicht", "data", "Wechselkurs", "detail", "text"));
        if (wechselkurs != null) {
